...
describe('The collapsible menu component', () => {
  test('will render for home', () => {
    const wrapper = shallow(<CollapsibleMenu pathname="/" menuClicked={jest.fn()} service="twitter" />);
    expect(toJson(wrapper)).toMatchSnapshot();
  });
  test('will render for my pins', () => {
    const wrapper = shallow(<CollapsibleMenu pathname="/pins" menuClicked={jest.fn()} service="twitter" />);
    expect(toJson(wrapper)).toMatchSnapshot();
  });
  ...
  test('will stay on the same page if users is selected again while on a profile page', () => {
    const wrapper = shallow(<CollapsibleMenu pathname="/some_profile" menuClicked={jest.fn()} service="twitter" />);
    const usersLink: EnzymePropSelector = wrapper.find('ForwardRef(MenuItem)').at(2);
